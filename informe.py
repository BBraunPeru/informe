...
class Informe(ft.UserControl):
    def build(self):
        # CAMPOS DE TEXTO
        self.inst = self.input_text(
            label="Institución", text_default="BBRAUN MEDICAL PERU"
        )
        self.serv = self.input_text(label="Servicio", text_default="STOCK")
        self.model = self.select_opcion(
            "Modelo",
            "Seleccione el modelo",
            ["I. SPACE", "P. SPACE", "P. SPACE PLUS", "COMPAC PLUS"],
        )
        self.serie = self.input_text(label="Serie", on_change=self.verify_sq)
        self.qr = self.input_text(label="QR")
        self.fecha = self.input_text(
            label="Fecha", text_default=str(datetime.date.today())
        )
        self.otros_eval = self.input_text(
            label="Otros_eval", text_default="Mnto.Preventivo", width=400
        )
        self.otros_act = self.input_text(label="Otros_act", width=200)
        self.porcentaje = self.input_text(
            label="Test Exact.(%)", text_default="0", width=150
        )
        ...
        self.comen_rec = self.input_text(
            ...
            width=600,
            text_default="Prueba de infusion y sensores OK",
        )
        ...
        self.dlg_modal = ft.AlertDialog(
            ...
            actions=[
                ...
                ft.TextButton(
                    "Intentar otra vez",
                    on_click=self.intent_new,
                    style=ft.ButtonStyle(
                        bgcolor=ft.colors.GREEN_400, color=ft.colors.BLACK
                    ),
                ),
            ],
        ...
        self.pb = ft.ProgressBar(width=400, color=ft.colors.GREEN_ACCENT)
        self.pb_value = ft.Text(value="0%", text_align="center")
        self.view_inform = ft.FloatingActionButton("Abrir Folder",visible=False,on_click=self.open_directory,bgcolor=ft.colors.GREEN_900,width=150,height=40)

        self.progreso = ft.Row(
            alignment="center",
            controls=[
                ft.Column(
                    horizontal_alignment="center",
                    controls=[self.pb_value, self.pb]
                ),
                self.view_inform
            ],
            visible=False,
        ...
        self.input_n_informe = self.input_text(
            label="Nro. Informe", text_default=self.get_n_informe()
        )
        ...
        self.app_bar = ft.Row(
            alignment="center", controls=[self.tabs, self.btn_make_informe]
        )
        ...
        self.cliente = ft.Column(
            ...
            controls=[
                ...
                ft.Row(
                    alignment="center",
                    controls=[self.check_btns(0, 2), self.input_n_informe],
                ),
            ],
        )
        ...
        self.equipo = ft.Column(
            [
                ...
                ft.Row(
                    alignment="center",
                    controls=[self.check_btns(14, 16), self.otros_eval],
                ),
            ]
        )
            ...
            controls=[
                ...
                ft.Column(
                    ...
                    controls=[
                        ft.Row(
                            alignment="center",
                            controls=[self.porcentaje, self.check_btns(24, 26)],
                        ),
                        ft.Row(
                            alignment="center",
                            controls=[
        ...
        self.repuestos = ft.Column(
            ...
            controls=[
                ...
                ft.Row(
                    alignment="center",
                    controls=[self.check_btns(48, 51), self.otros_rep],
                ),
            ],
        )
        ...
        self.estado_final = ft.Column(
            horizontal_alignment="center",
            controls=[self.check_btns(51, 54), self.comen_rec, self.upload_file],
        )
    ...
    def open_directory(self,event):
        subprocess.Popen(f"explorer C:\\Users\\BRIX\\Desktop\\Informe-Tecnico-main\\informes\\informes_{self.fecha.value}")


    def crear_directorio(self):
        directorio = f"./informes/informes_{self.fecha.value}"
    ...
    def launch_make_informe(self, e):
        directorio = self.crear_directorio()
        self.n_informe = int(self.input_n_informe.value)
        ...
        super().update()
        self.make_informe(
            serie=self.serie.value, qr=self.qr.value, directorio=directorio
        )
        self.set_n_informe(str(self.n_informe))
        self.pb.value = 1
        # conn.upload_to_database((self.model.value,self.serv.value,self.serie.value,self.qr.value,"OPERATIVO"))
        self.pb_value.value = "100%"
        self.view_inform.visible = True
        super().update()


    def make_informe(self, serie, qr, directorio,model = None,status = "OPERATIVO",fecha = None):
        if not model is None:
            modelo = model

            if status == "OPERATIVO":
                if modelo == "P. SPACE" or modelo == "P. C. PLUS" or modelo == "ENTEROPORT":
                    check_list_state[26] = False
            
        else:
            modelo = self.model.value


        if not fecha is None:
            date = fecha
        else:
            date = self.fecha.value
        textos = [
            self.inst.value,
            self.serv.value,
            modelo,
        ...
        pdf.make_pdf(
            textos=textos,
            checks=check_list_state,
            n_informe=self.n_informe,
            name_dir=directorio,
        )
        self.n_informe += 1
    ...
    def pick_files_result(self, e: ft.FilePickerResultEvent):
        ...
        if not xls_file is None:
            ...
            data = pd.read_excel(ruta,dtype={"SERIE":str,"QR":str})
            print(data)

            self.progreso.visible = True

            for sn, qr, i,model,status,fecha in zip(
                data["SERIE"], data["QR"], range(1, len(data["SERIE"]) + 1),data["MODELO"],data["ESTADO"],data["FECHA"]
            ):
                print(sn, qr)
                self.make_informe(serie=str(sn), qr=str(qr), directorio=directorio,model = model,status=status,fecha=fecha)
                valor = i * (1 / len(data["SERIE"]))
                self.pb.value = valor
            ...
            #conn.upload_to_database_masive([tuple(str(j) for j in x) for x in data.to_records(index=False)])
            self.view_inform.visible = True
            super().update()
